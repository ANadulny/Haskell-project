...
-- fillWithMaxAndMinValues :: Pyramids -> Board -> Board
-- fillWithMaxAndMinValues (Pyramids top bottom left right) board = do
--         fillWithMaxAndMinValuesFromArray top board
--         -- add let tj. result from top -> bottom -> left -> right <- i to jest ost. rezultat zwracany

-- fillWithMaxAndMinValuesFromArray :: [Maybe Int] -> Board -> Board
-- fillWithMaxAndMinValuesFromArray (x:xs) (Board rows) = Board  (transpose rows)
-- -- findIndex (\x -> 5**x > 10000) [2,4,6,8] 
-- -- (\lambda ) [top, bottom, left, right]
-- -- fillWithMaxAndMinValuesFromArray (x:xs) board = | x == getSize board = ...
-- --                                                 | x == 1 = ...
-- --                                                 | otherwise = ...
...
isLineOk (Pyramids t b l r) (Board rows) (x,y) = fromLeft  && fromRight && fromTop && fromBottom
    where
